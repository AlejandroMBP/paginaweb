    ...
    return (
        <footer className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white py-10 px-6 mt-8 shadow-lg border-t border-gray-700">
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div>
                    <h3 className="text-xl font-bold mb-4 uppercase text-gray-100 border-b border-gray-700 pb-2 hover:text-secondary transition">Contacto</h3>
                    {loading ? (
                        ...
                        <ul className="space-y-3 text-gray-300">
                            ...
                            {contactInfo?.institucion_celular2 && (
                                <li
                                    className="flex items-center gap-3 hover:text-secondary transition cursor-pointer"
                                    onClick={() => copyToClipboard(contactInfo.institucion_celular2?.toString() || '')}
                                >
                                    <FaPhone className="text-blue-400 text-xl" />
                                    {contactInfo.institucion_celular2}
                                </li>
                            ...
                            {contactInfo.institucion_correo1 && (
                                <li className="flex items-center gap-3 hover:text-secondary transition cursor-pointer" onClick={() => copyToClipboard(contactInfo.institucion_correo1)}>
                                    <FaEnvelope className="text-yellow-400 text-xl" />
                                    {contactInfo.institucion_correo1}
                                </li>
                            )}
                            {contactInfo.institucion_correo2 && (
                                <li className="flex items-center gap-3 hover:text-secondary transition cursor-pointer" onClick={() => copyToClipboard(contactInfo.institucion_correo2)}>
                                    <FaEnvelope className="text-yellow-400 text-xl" />
                                    {contactInfo.institucion_correo2}
                                </li>
                            )}
                ...
                <div>
                    <h3 className="text-xl font-bold mb-4 uppercase text-gray-100 border-b border-gray-700 pb-2 hover:text-secondary transition">Síguenos</h3>
                    {loading ? (
                        <p className="text-gray-400">Cargando...</p>
                    ) : contactInfo ? (
                        <div className="flex space-x-6">
                            {contactInfo.institucion_facebook && (
                                <a href={contactInfo.institucion_facebook} target="_blank" rel="noopener noreferrer"
                                    className="text-blue-500 hover:text-secondary transition duration-300 text-3xl">
                                    <FaFacebook />
                                </a>
                            )}
                            {contactInfo.institucion_twitter && (
                                <a href={contactInfo.institucion_twitter} target="_blank" rel="noopener noreferrer"
                                    className="text-blue-400 hover:text-secondary transition duration-300 text-3xl">
                                    <FaTwitter />
                                </a>
                            )}
                            {contactInfo.institucion_youtube && (
                                <a href={contactInfo.institucion_youtube} target="_blank" rel="noopener noreferrer"
                ...
                <div className="text-center md:text-right">
                    <p className="text-gray-400 font-semibold text-sm hover:text-secondary transition">
                        © {new Date().getFullYear()} <a href="#" className="text-gray-300 hover:text-secondary font-bold transition">Dev - MABP</a>. <br />U-ticTodos los derechos reservados.
                    </p>
                </div>
            </div>
        </footer>
